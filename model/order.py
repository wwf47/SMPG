...
def get_order(path_weight, seed, can, seed_num, rel, rel_st, triple, start, end, pma):
    ...
    for i in trange(len(can), desc="candidate"):
        can_tmp = 0
        for j in range(seed_num):  # seed
            ...
            for k in path_weight:
                ...
                if k not in pma:
                    matrix = path_matrix(k, merge, seed_set, rel, rel_st, triple)
                    start[k] = matrix[0]  # start list
                    end[k] = matrix[1]  # end list
                    pma[k] = matrix[2]  # matrix
                if (can[i] in end[k]) and (seed[j] in start[k]):
                    flag = pma[k].toarray()[start[k].index(seed[j]), end[k].index(can[i])]  # have relation
                if flag != 0:
